# photostore-client

React + Vite SPA for the photostore AWS Serverless API (Cognito, API Gateway, S3, DynamoDB).

## Stack

- **React 19** + **Vite** + TypeScript
- **TanStack Query** — API data fetching and mutations
- **AWS Amplify Auth** — Cognito sign-in; JWT sent to protected routes
- **Prettier** — code formatting

## Setup

```bash
yarn install
cp .env.example .env   # if .env is missing
yarn dev
```

Configure `.env` from your `yarn deploy` outputs in the monorepo root:

| Variable                   | Source                           |
| -------------------------- | -------------------------------- |
| `VITE_API_URL`             | `HttpApiUrl` (no trailing slash) |
| `VITE_USER_POOL_ID`        | `UserPoolId`                     |
| `VITE_USER_POOL_CLIENT_ID` | `UserPoolClientId`               |
| `VITE_IDENTITY_POOL_ID`    | `IdentityPoolId`                 |
| `VITE_AWS_REGION`          | e.g. `us-east-1`                 |

## Scripts

| Command             | Description                                      |
| ------------------- | ------------------------------------------------ |
| `yarn dev`          | Start dev server (default http://localhost:5173) |
| `yarn build`        | Production build                                 |
| `yarn lint`         | ESLint                                           |
| `yarn format`       | Prettier write                                   |
| `yarn format:check` | Prettier check                                   |

## API flow

Authenticated routes match the backend README:

1. `POST /photos/upload-url` — presigned S3 PUT URL
2. `PUT` to S3 (direct, no JWT)
3. `POST /photos` — save metadata
4. `GET /photos` — list your photos

`GET /hello` is public and shown as an API health indicator when the app loads.

### Guest uploads (before sign-in)

1. Amplify resolves a **Cognito Identity Pool** id (`allowGuestAccess: true`).
2. `POST /guest/photos/upload-url` → S3 PUT → `POST /guest/photos` (max **2** photos).
3. On sign-in or sign-up confirm, `POST /photos/merge` moves guest objects to `users/{sub}/`.

## Cognito users

Create and confirm a user in the photostore pool (see photostore `README.md`), then sign in with email + password in the app.
